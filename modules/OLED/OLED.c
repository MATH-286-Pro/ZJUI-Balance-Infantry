/**
  ...
  *             0.96OLED使用SSD1306驱动器，本文仅包括初始化函数以及其他OLED设置函数，GRAM操作函数，OLED显示数字，字符，字符串函数
...
static uint8_t OLED_GRAM[128][8]; //显存缓存数组
...
/**
  ...
  */
/**
  * @brief          写入数据或指令到OLED，如果使用的是SPI，请重写这个函数
  * @param[in]      dat: 要写入的字节
  * @param[in]      cmd: OLED_CMD 代表写入的是命令; OLED_DATA 代表写入的是数据
  * @retval         none
  */
...
/**
  ...
  */
/**
  * @brief          操作 OLED_GRAM 显存缓冲数组(128*8char数组)
  * @param[in]      pen: 操作类型
                    PEN_CLEAR: 设置为0x00
                    PEN_WRITE: 设置为0xff
                    PEN_INVERSION: 按位取反
...
/**
  ...
  */
/**
  * @brief          操作 OLED_GRAM 显存缓冲数组中的一个位，相当于操作屏幕的一个点
...
/**
  ...
  */
/**
  * @brief          发送数据到OLED的操作 OLED_GRAM 显存缓冲数组
  * @param[in]      none
  * @retval         none
  */
...
void OLED_clear()
{
  OLED_operate_gram(PEN_CLEAR); //清空 OLED_GRAM 显存
  OLED_refresh_gram();
}
...
void OLED_showBMP_gram(uint8_t BMP[128][8])
{
  uint8_t x,y; 
  for(x=0;x<128;x+=1)
  {
...
void OLED_mask_write_gram(uint8_t xstart, uint8_t ystart, uint8_t BMP[128][8])
{
  uint8_t x,y;
  bool_t z;
...
void OLED_mask_clear_gram(uint8_t xstart, uint8_t ystart, uint8_t BMP[128][8])
{
  uint8_t x,y;
  bool_t z;
...
/**
  ...
  * @param  Line   起始行位置,范围Χ:1~4
  * @param  Column 起始列位置,范围Χ:1~16
  * @param  Number 要显示的数字，范围Χ:0~4294967295
  * @param  Length 要显示的数字长度，范围Χ:1~10
  * @retval 无
  */
...
/**
  * @brief  OLED   显示数字(十进制，带符号整数)
  * @param  Line   起始行位置,范围Χ:1~4
  * @param  Column 起始列位置,范围Χ:1~16
  * @param  Number 要显示的数字，范围Χ:-2147483648~2147483647
  * @param  Length 要显示的数字长度，范围Χ:1~10
  * @retval 无
  */
