...
uint8_t MI_TESTID = 253; //2024.3.5测试
...
{
    CAN_TxHeader_MI.DLC = 8;          //数据长度8位
    CAN_TxHeader_MI.IDE = CAN_ID_EXT; //扩展ID
...
void MI_motor_Enable(MI_Motor_s* hmotor)
{
    ...
    //hmotor->EXT_ID.data = MI_TESTID; //2024.3.5测试
    hmotor->EXT_ID.res = 0;
    for(uint8_t i=0; i<8; i++)
    {
        hmotor->txdata[i]=0; //让8位数据全都变成0
    }
